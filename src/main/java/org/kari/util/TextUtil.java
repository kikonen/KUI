...
public final class TextUtil {
    ...
    private static final TIntObjectHashMap<String> mHtmlEncodeMap = new TIntObjectHashMap<String>();
    ...
    /**
     * Convert pText to HTML, unless it already is
     */
    public static final String toHTML(final String pText) {
        String result = pText;
        if (!pText.startsWith("<html>")) {
            result = pText.replaceAll("\n", "<br>");
    ...
    /**
     * Expand all system properties in pText
     * 
     * @param pText Text, if null nothing is done
     * @return Text, null if pText is null
     * 
     * @see System#getProperty(String)
     */
    public static String expand(String pText) {
        return expand(pText, START_VAR, END_VAR, null, true);
    }
    
    public static String expand(
        final String pText,
        final String pVarPrefix,
        final String pVarPostfix,
        final Map<String, String> pEnv,
        final boolean pSystem) 
    {
        StringBuilder sb = null;
        if (pText != null) {
            int postfixLen = pVarPostfix.length();
            int prefixLen = pVarPrefix.length();
            
            boolean done = false;
            int prevIdx = 0;
            while (!done) {
                int startIdx = pText.indexOf(pVarPrefix, prevIdx);
                if (startIdx != -1) {
                    int endIdx = pText.indexOf(pVarPostfix, startIdx + prefixLen);
                    if (endIdx != -1) {
                        String varName = pText.substring(startIdx + prefixLen, endIdx);
                        String value = null;
                        if (pEnv != null) {
                            value = pEnv.get(varName);
                        }
                        if (value == null && pSystem) {
                            value = System.getProperty(varName);
                        }
                        if (sb == null) {
                            sb = new StringBuilder(pText.length() + 200);
                        }
                        sb.append(pText.substring(prevIdx, startIdx));
                        if (value != null) {
                            sb.append(value);
                        }
                        prevIdx = endIdx + postfixLen;
                    } else {
                        done = true;
                    }
                } else {
            ...
            if (sb != null && prevIdx < pText.length()) {
                sb.append(pText.substring(prevIdx));
            }
        }
     ...
     * @since 1.0
     */
    private static String encode(String source, TIntObjectHashMap<String> encodingTable)
    {
        if (null == source)
        {
            return null;
